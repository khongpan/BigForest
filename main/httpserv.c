...
http_server_netconn_serve(struct netconn *conn)
{
  ...
  if (err == ERR_OK) {
    ...
    if (buflen>=5 &&
	  ...
	  cmd_str=strtok(&buf[5]," ");
	  printf("cmd=%s\n",cmd_str);  
	  ret_str=malloc(80);
	  
	  char *spc_str;
	  while((spc_str=strstr(cmd_str,"%20"))!=NULL)
	  {
		  
		  spc_str[0]=spc_str[1]=spc_str[2]=' ';
		  spc_str+=3;
	  }
	  //printf("cmd=%s\n",cmd_str);
	  cmd_process_str(cmd_str,ret_str);
	  
	  ESP_LOGI(TAG,"response");
	  netconn_write(conn, ret_str, strlen(ret_str), NETCONN_NOCOPY);
	  ESP_LOGI(TAG,"%s",ret_str);
	  free(ret_str);
...
static void generate_json() {
	...
	cJSON_AddStringToObject(d, "myName", "NODE_MCU_002");
	cJSON_AddNumberToObject(d, "temperature", 99.0);
	cJSON_AddNumberToObject(d, "humidity", 99.0);
	...
	cJSON_AddNumberToObject(info, "time", time(NULL));

	while (1) {
		
		cJSON_ReplaceItemInObject(d, "temperature",
				cJSON_CreateNumber(sht31arp_read_temp()));
		cJSON_ReplaceItemInObject(d, "humidity",
				cJSON_CreateNumber(sht31arp_read_humid()));
